...
namespace components {
    ...
    std::vector<v2>
    compute_distance(const std::shared_ptr<joined_location> loc, const std::vector<v2> &patterns, const absl::flat_hash_map<std::string, OGRPoint> &centroids) {
    ...
    std::vector<v2>
    expandRow(const std::shared_ptr<weekly_pattern> row, const std::shared_ptr<std::vector<std::pair<std::string, std::uint16_t>>> cbg_visits) {
    ...
    hpx::future<void> RowProcessor::processRow(const std::shared_ptr<weekly_pattern> row) {
...
//            const auto offset = calculate_cbg_offset(cbg_offsets, jl.location_cbg);
...
//                    }
    ...
    hpx::future<void> RowProcessor::handle_row(const std::shared_ptr<weekly_pattern> row, const std::shared_ptr<joined_location> jl) {
        const auto visits = std::make_shared<std::vector<std::pair<std::string, std::uint16_t>>>(extract_cbg_visits(row));
        auto centroid_future = get_centroid_map(visits);
        hpx::future<std::vector<v2>> row_future = hpx::async(&expandRow, row, visits);

        auto distance_res = dataflow(unwrapping(&compute_distance), hpx::make_ready_future(jl), row_future, centroid_future);
    ...
    hpx::future<absl::flat_hash_map<std::string, OGRPoint>> RowProcessor::get_centroid_map(const std::shared_ptr<std::vector<std::pair<std::string, std::uint16_t>>> visits) {
        ...
        std::for_each(expanded_rows.begin(), expanded_rows.end(),
                      [this](const v2 &expanded_row) {
                          ...
                          _matricies.insert(t_offset,
                                           calculate_cbg_offset(_offset_map, expanded_row.location_cbg),
                                           calculate_cbg_offset(_offset_map, expanded_row.visit_cbg),
