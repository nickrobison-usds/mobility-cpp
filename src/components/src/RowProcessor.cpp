...
namespace components {
    ...
    std::vector<v2>
    compute_distance(const joined_location &loc, const std::vector<v2> &patterns, const absl::flat_hash_map<std::string, OGRPoint> &centroids) {
...
//            const auto offset = calculate_cbg_offset(cbg_offsets, jl.location_cbg);
    ...
    hpx::future<void> RowProcessor::handle_row(const weekly_pattern &row, const joined_location &jl) {
        const auto visits = extract_cbg_visits(row);
        auto centroid_future = get_centroid_map(row, visits);
    ...
    hpx::future<absl::flat_hash_map<std::string, OGRPoint>> RowProcessor::get_centroid_map(const weekly_pattern &row, const std::vector<std::pair<std::string, std::uint16_t>> &visits) {
        auto cbg_future = hpx::async([&row, &visits]() {
