...
namespace mt::coordinates {
    std::vector<LocaleLocator::value>
    LocaleTiler::tile(const Coordinate2D &min, const Coordinate2D &max, const std::array<std::size_t, 2> &stride) {

        const std::size_t stride_x = stride[0];
        const std::size_t stride_y = stride[1];

        std::vector<LocaleLocator::value> results;
        ...
        for (std::size_t i = min.get_dim0(); i < max.get_dim0(); i += stride_x) {
            ...
            for (std::size_t j = min.get_dim1(); j < max.get_dim1(); j += stride_y) {
                ...
                results.emplace_back(mt_tile(current_min, max_point), loc_count);
